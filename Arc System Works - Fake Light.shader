...
////// Emissive:
                ...
                float3 finalColor = emissive + ((lerp((_ShadowBrightness*_SSS_var.rgb),float3(_LightIntensity,_LightIntensity,_LightIntensity),clamp(floor(clamp((max(0,dot((_LightDirection.rgb*100000000000.0),normalDirection))*_shadow_push),0,1)),0,5))*_LightColor.rgb)*_Diffuse_var.rgb);
