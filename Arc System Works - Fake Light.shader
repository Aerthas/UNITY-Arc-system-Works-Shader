...
Shader "Arc System Works/Fake Light" {
    Properties {
        ...
        _ShinePush ("Shine Push", Float ) = 0.87
...
////// Emissive:
                ...
                float3 finalColor = emissive + (((lerp((_ShadowBrightness*_SSS_var.rgb),float3(_LightIntensity,_LightIntensity,_LightIntensity),clamp(floor(clamp(max(0,dot((100000000000.0*_LightDirection.rgb),normalDirection)),0,1)),0,5))+lerp(0.0,_ShineIntensity,clamp(floor(clamp((max(0,dot((_LightDirection.rgb*_ShinePush),normalDirection))*_ShinePush),0,1)),0,5)))*_LightColor.rgb)*_Diffuse_var.rgb);
