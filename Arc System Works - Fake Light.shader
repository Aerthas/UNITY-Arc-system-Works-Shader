...
////// Emissive:
                ...
                float3 finalColor = emissive + (((lerp((_ShadowBrightness*_SSS_var.rgb),float3(_LightIntensity,_LightIntensity,_LightIntensity),clamp(floor(clamp((_ShadowPush*max(0,dot((_ShadowPush*_LightDirection.rgb),normalDirection))),0,1)),0,5))+lerp(0.0,_ShineBrightness,clamp(floor(clamp((max(0,dot((_LightDirection.rgb*_ShinePush),normalDirection))*_ShinePush),0,1)),0,5)))*_LightColor.rgb)*node_8694);
