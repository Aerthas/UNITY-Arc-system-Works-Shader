...
Shader "Arc System Works/Real Light" {
    Properties {
        ...
        _ShineBrightness ("Shine Brightness", Float ) = 0.7
    }
    SubShader {
        Tags {
            "RenderType"="Opaque"
...
////// Emissive:
                ...
                float3 finalColor = emissive + (((lerp((_ShadowBrightness*_SSS_var.rgb),float3(_LightIntensity,_LightIntensity,_LightIntensity),clamp(floor(clamp((_ShadowPush*node_7383),0,1)),0,5))+lerp(0.0,_ShineBrightness,clamp(floor(clamp((node_7383*_ShinePush),0,2)),0,5)))*_LightColor0.rgb)*node_5502);
...
////// Lighting:
                ...
                float4 _Diffuse_var = tex2D(_Diffuse,TRANSFORM_TEX(i.uv0, _Diffuse));
                float3 node_5502 = (_Diffuse_var.rgb*_DiffuseTint.rgb);
                float3 finalColor = (((lerp((_ShadowBrightness*_SSS_var.rgb),float3(_LightIntensity,_LightIntensity,_LightIntensity),clamp(floor(clamp((_ShadowPush*node_7383),0,1)),0,5))+lerp(0.0,_ShineBrightness,clamp(floor(clamp((node_7383*_ShinePush),0,2)),0,5)))*_LightColor0.rgb)*node_5502);
