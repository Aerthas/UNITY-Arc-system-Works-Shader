...
////// Emissive:
                ...
                float3 finalColor = emissive + ((lerp((_ShadowBrightness*_SSS_var.rgb),float3(_LightIntensity,_LightIntensity,_LightIntensity),clamp(floor(clamp(((max(0,dot(lightDirection,normalDirection))*attenuation)*_shadow_push),0,1)),0,5))*_LightColor0.rgb)*_Diffuse_var.rgb);
...
////// Lighting:
                ...
                float4 _SSS_var = tex2D(_SSS,TRANSFORM_TEX(i.uv0, _SSS));
                float4 _Diffuse_var = tex2D(_Diffuse,TRANSFORM_TEX(i.uv0, _Diffuse));
                float3 finalColor = ((lerp((_ShadowBrightness*_SSS_var.rgb),float3(_LightIntensity,_LightIntensity,_LightIntensity),clamp(floor(clamp(((max(0,dot(lightDirection,normalDirection))*attenuation)*_shadow_push),0,1)),0,5))*_LightColor0.rgb)*_Diffuse_var.rgb);
