...
Shader "Arc System Works/Real Light" {
    Properties {
        ...
        _BodyLineBrightness ("Body Line Brightness", Float ) = 1
    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                return fixed4((_Diffuse_var.rgb*_OutlineColor.rgb*_OutlineBrightness),0);
            }
            ENDCG
        }
        Pass {
            ...
            uniform float _BodyLineBrightness;
            struct VertexInput {
                float4 vertex : POSITION;
                float3 normal : NORMAL;
                float2 texcoord0 : TEXCOORD0;
...
////// Emissive:
                ...
                float3 emissive = (node_2455+(((node_2455-node_5252)*_BodyLinesColor.rgb*(-1.0))*_BodyLineBrightness));
                float4 _SSS_var = tex2D(_SSS,TRANSFORM_TEX(i.uv0, _SSS));
                float node_7383 = (attenuation*max(0,dot(lightDirection,normalDirection)));
                float3 finalColor = emissive + (((lerp((_ShadowBrightness*_SSS_var.rgb),float3(_LightIntensity,_LightIntensity,_LightIntensity),clamp(floor(clamp((_ShadowPush*node_7383),0,1)),0,5))+lerp(0.0,_ShineBrightness,clamp(floor(clamp((node_7383*_ShinePush),0,2)),0,5)))*_LightColor0.rgb)*_Diffuse_var.rgb);
...
////// Lighting:
                ...
                float node_7383 = (attenuation*max(0,dot(lightDirection,normalDirection)));
                float4 _Diffuse_var = tex2D(_Diffuse,TRANSFORM_TEX(i.uv0, _Diffuse));
                float3 finalColor = (((lerp((_ShadowBrightness*_SSS_var.rgb),float3(_LightIntensity,_LightIntensity,_LightIntensity),clamp(floor(clamp((_ShadowPush*node_7383),0,1)),0,5))+lerp(0.0,_ShineBrightness,clamp(floor(clamp((node_7383*_ShinePush),0,2)),0,5)))*_LightColor0.rgb)*_Diffuse_var.rgb);
