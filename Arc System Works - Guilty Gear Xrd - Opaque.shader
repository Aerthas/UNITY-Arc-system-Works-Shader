...
Shader ".Aerthas/Arc System Works/Game Select/Guilty Gear Xrd/Opaque v7.2.0"
{
	Properties
	{
	...
	LOD 100
		...
		Pass
		{
			...
			float3 ViewMatrix0375_g1462(  )
			{
				return UNITY_MATRIX_V[0];
			}
			
			float3 ViewMatrix1373_g1462(  )
			{
				return UNITY_MATRIX_V[1];
			}
			
			float3 StereoCameraViewPosition30_g1460(  )
			{
			...
			float3 StereoCameraViewDirection32_g1460( float3 worldPos )
			{
				#if UNITY_SINGLE_PASS_STEREO
				float3 cameraPos = float3((unity_StereoWorldSpaceCameraPos[0]+ unity_StereoWorldSpaceCameraPos[1])*.5); 
				#else
			...
			float4 frag (v2f i ) : SV_Target
			{
				...
				float3 localViewMatrix0375_g1462 = ViewMatrix0375_g1462();
				float3 normalizeResult384_g1462 = normalize( localViewMatrix0375_g1462 );
				float3 ase_worldPos = i.ase_texcoord3.xyz;
				float3 temp_output_380_0_g1462 = ( float3( 0,0,0 ) + ase_worldPos );
				float3 localViewMatrix1373_g1462 = ViewMatrix1373_g1462();
				float3 normalizeResult376_g1462 = normalize( localViewMatrix1373_g1462 );
				float3 localStereoCameraViewPosition30_g1460 = StereoCameraViewPosition30_g1460();
				float3 rotatedValue385_g1462 = RotateAroundAxis( temp_output_380_0_g1462, localStereoCameraViewPosition30_g1460, normalizeResult376_g1462, radians( ( _ViewDirOffsetYaw * -1.0 ) ) );
				float3 rotatedValue387_g1462 = RotateAroundAxis( temp_output_380_0_g1462, rotatedValue385_g1462, normalize( normalizeResult384_g1462 ), radians( ( _ViewDirOffsetPitch * 1.0 ) ) );
				float3 normalizeResult389_g1462 = normalize( ( rotatedValue387_g1462 - temp_output_380_0_g1462 ) );
				float3 normalizeResult38_g1460 = normalize( normalizeResult389_g1462 );
				float3 appendResult15_g1461 = (float3(( cos( ( ( _FakeLightDirX / 180.0 ) * UNITY_PI ) ) * sin( ( ( _FakeLightDirY / 180.0 ) * UNITY_PI ) ) * -1.0 ) , sin( ( ( _FakeLightDirX / 180.0 ) * UNITY_PI ) ) , ( cos( ( ( _FakeLightDirX / 180.0 ) * UNITY_PI ) ) * cos( ( ( _FakeLightDirY / 180.0 ) * UNITY_PI ) ) * -1.0 )));
				float3 normalizeResult2_g1461 = normalize( appendResult15_g1461 );
				float3 normalizeResult26_g1460 = normalize( normalizeResult2_g1461 );
				float3 ifLocalVar3_g1460 = 0;
				if( FallbackLightDirection563 > 0.0 )
				ifLocalVar3_g1460 = normalizeResult38_g1460;
				else if( FallbackLightDirection563 == 0.0 )
				ifLocalVar3_g1460 = normalizeResult26_g1460;
				...
				float3 ifLocalVar8_g1460 = 0;
				if( IsThereWorldLight450 > 0.0 )
				ifLocalVar8_g1460 = worldSpaceLightDir;
				else if( IsThereWorldLight450 == 0.0 )
				ifLocalVar8_g1460 = ifLocalVar3_g1460;
				float3 ifLocalVar9_g1460 = 0;
				if( LightDirectionSetting561 > 0.0 )
				ifLocalVar9_g1460 = ifLocalVar3_g1460;
				else if( LightDirectionSetting561 == 0.0 )
				ifLocalVar9_g1460 = ifLocalVar8_g1460;
				float3 normalizeResult48_g1460 = normalize( ifLocalVar9_g1460 );
				float3 worldPos32_g1460 = ase_worldPos;
				float3 localStereoCameraViewDirection32_g1460 = StereoCameraViewDirection32_g1460( worldPos32_g1460 );
				float3 normalizeResult51_g1460 = normalize( ( normalizeResult48_g1460 + localStereoCameraViewDirection32_g1460 ) );
				float dotResult50_g1460 = dot( normalizedWorldNormal , normalizeResult51_g1460 );
				float blendOpSrc34_g1458 = ILMB576;
				float blendOpDest34_g1458 = dotResult50_g1460;
				float clampResult87_g1463 = clamp( ( ( saturate( ( 1.0 - ( ( 1.0 - blendOpDest34_g1458) / max( blendOpSrc34_g1458, 0.00001) ) ) )) > ( 1.0 - _SpecularSize ) ? 1.0 : 0.0 ) , 0.0 , 1.0 );
				float4 temp_cast_28 = (clampResult87_g1463).xxxx;
				float4 blendOpSrc30_g1466 = Base12_g1466;
				float4 blendOpDest30_g1466 = temp_cast_28;
				float4 lerpBlendMode30_g1466 = lerp(blendOpDest30_g1466,min( blendOpSrc30_g1466 , blendOpDest30_g1466 ),0.9);
				float3 desaturateInitialColor9_g1469 = ( ( ILMR573 * ( saturate( lerpBlendMode30_g1466 )) ) * float4( 1,1,1,0 ) * _SpecularIntensity * _SpecularTint ).rgb;
				float desaturateDot9_g1469 = dot( desaturateInitialColor9_g1469, float3( 0.299, 0.587, 0.114 ));
				float3 desaturateVar9_g1469 = lerp( desaturateInitialColor9_g1469, desaturateDot9_g1469.xxx, ( 1.0 - _SpecularSaturation ) );
				float4 temp_output_95_0_g1466 = ( float4( desaturateVar9_g1469 , 0.0 ) + Base12_g1466 );
				float4 LightColor779 = _LightColor;
				float3 desaturateInitialColor9_g1467 = ( temp_output_95_0_g1466 * LightColor779 * _BaseIntensity * _BaseTint ).rgb;
				float desaturateDot9_g1467 = dot( desaturateInitialColor9_g1467, float3( 0.299, 0.587, 0.114 ));
				float3 desaturateVar9_g1467 = lerp( desaturateInitialColor9_g1467, desaturateDot9_g1467.xxx, ( 1.0 - _BaseSaturation ) );
				float4 AmbientColor780 = _AmbientColor;
				float3 SSS578 = temp_output_993_0[1];
				float4 SSS14_g1466 = float4( SSS578 , 0.0 );
				float4 temp_output_97_0_g1466 = ( temp_output_95_0_g1466 * SSS14_g1466 );
				float3 desaturateInitialColor9_g1470 = ( temp_output_97_0_g1466 * float4( 1,1,1,0 ) * _Shadow1Intensity * _Shadow1Tint ).rgb;
				float desaturateDot9_g1470 = dot( desaturateInitialColor9_g1470, float3( 0.299, 0.587, 0.114 ));
				float3 desaturateVar9_g1470 = lerp( desaturateInitialColor9_g1470, desaturateDot9_g1470.xxx, ( 1.0 - _Shadow1Saturation ) );
				float4 blendOpSrc51_g1466 = float4( 0,0,0,0 );
				float4 blendOpDest51_g1466 = temp_output_97_0_g1466;
				float4 lerpBlendMode51_g1466 = lerp(blendOpDest51_g1466,2.0f*blendOpDest51_g1466*blendOpSrc51_g1466 + blendOpDest51_g1466*blendOpDest51_g1466*(1.0f - 2.0f*blendOpSrc51_g1466),0.5);
				float3 desaturateInitialColor9_g1468 = ( ( saturate( lerpBlendMode51_g1466 )) * float4( 1,1,1,0 ) * _Shadow2Intensity * _Shadow2Tint ).rgb;
				float desaturateDot9_g1468 = dot( desaturateInitialColor9_g1468, float3( 0.299, 0.587, 0.114 ));
				float3 desaturateVar9_g1468 = lerp( desaturateInitialColor9_g1468, desaturateDot9_g1468.xxx, ( 1.0 - _Shadow2Saturation ) );
				float VertexR33_g1463 = i.ase_color.r;
				float dotResult46_g1460 = dot( ifLocalVar9_g1460 , normalizedWorldNormal );
				float temp_output_36_0_g1458 = ( ( dotResult46_g1460 + _GlobalLightPush ) * i.ase_color.r );
				float ILMG574 = break292.y;
				float temp_output_1_0_g1458 = ILMG574;
				float temp_output_1064_0 = ( temp_output_36_0_g1458 * temp_output_1_0_g1458 );
				float NdL20_g1463 = temp_output_1064_0;
				float temp_output_9_0_g1463 = _Shadow1Push;
				float temp_output_11_0_g1463 = _Shadow2Push;
				float clampResult15_g1463 = clamp( temp_output_9_0_g1463 , temp_output_11_0_g1463 , temp_output_9_0_g1463 );
				float RealShadow1Push16_g1463 = clampResult15_g1463;
				float clampResult17_g1463 = clamp( temp_output_11_0_g1463 , temp_output_11_0_g1463 , temp_output_9_0_g1463 );
				float RealShadow2Push18_g1463 = clampResult17_g1463;
				float temp_output_42_0_g1463 = max( max( ( 1.0 - ( VertexR33_g1463 > _Shadow2VertexRThreshold ? 1.0 : 0.0 ) ) , ( NdL20_g1463 < RealShadow2Push18_g1463 ? 1.0 : 0.0 ) ) , ( ILMG574 < _PermanentShadowThreshold ? 1.0 : 0.0 ) );
				float temp_output_43_0_g1463 = ( max( ( 1.0 - ( VertexR33_g1463 > _Shadow1VertexRThreshold ? 1.0 : 0.0 ) ) , min( ( NdL20_g1463 < RealShadow1Push16_g1463 ? 1.0 : 0.0 ) , ( RealShadow1Push16_g1463 >= RealShadow2Push18_g1463 ? 1.0 : 0.0 ) ) ) - temp_output_42_0_g1463 );
				float3 lerpResult100_g1466 = lerp( desaturateVar9_g1470 , desaturateVar9_g1468 , ( 1.0 - temp_output_43_0_g1463 ));
				float clampResult90_g1463 = clamp( ( 1.0 - ( temp_output_43_0_g1463 + temp_output_42_0_g1463 ) ) , 0.0 , 1.0 );
				float temp_output_39_0_g1466 = clampResult90_g1463;
				float4 lerpResult101_g1466 = lerp( float4( desaturateVar9_g1467 , 0.0 ) , ( AmbientColor780 * float4( lerpResult100_g1466 , 0.0 ) ) , ( 1.0 - temp_output_39_0_g1466 ));
				float EnableRimlight82_g1463 = _EnableRimlight;
				float dotResult31_g1460 = dot( normalizedWorldNormal , localStereoCameraViewDirection32_g1460 );
				...
				float NdV48_g1463 = ( ( 1.0 - ( dotResult31_g1460 + ( 1.0 - _RimlightSize ) ) ) * ( 1.0 - ( saturate( abs( blendOpSrc15_g1458 - blendOpDest15_g1458 ) )) ) );
				float clampResult91_g1463 = clamp( temp_output_42_0_g1463 , 0.0 , 1.0 );
				float clampResult88_g1463 = clamp( ( ceil( ( ( NdV48_g1463 > 0.1 ? 1.0 : 0.0 ) * BaseA580 ) ) - clampResult91_g1463 ) , 0.0 , 1.0 );
				float temp_output_94_0_g1463 = ( EnableRimlight82_g1463 == 1.0 ? clampResult88_g1463 : 0.0 );
				float3 desaturateInitialColor9_g1471 = ( ( Base12_g1466 * temp_output_94_0_g1463 ) * float4( 1,1,1,0 ) * _RimlightIntensity * _RimlightTint ).rgb;
				float desaturateDot9_g1471 = dot( desaturateInitialColor9_g1471, float3( 0.299, 0.587, 0.114 ));
				float3 desaturateVar9_g1471 = lerp( desaturateInitialColor9_g1471, desaturateDot9_g1471.xxx, ( 1.0 - _RimlightSaturation ) );
				float4 ColorOut1025 = ( lerpResult101_g1466 + float4( desaturateVar9_g1471 , 0.0 ) );
				...
				float4 temp_output_12_0_g1473 = max( float4( 0,0,0,0 ) , temp_output_11_0_g1473 );
				#ifdef UNITY_PASS_FORWARDADD
				float4 staticSwitch9_g1473 = temp_output_12_0_g1473;
				#else
				float4 staticSwitch9_g1473 = ifLocalVar3_g1473;
				#endif
				float4 GlobalLightColor944 = staticSwitch9_g1473;
				float EmissionRampMask920 = ( ( ( temp_output_94_0_g1463 * _RimlightEmissiveToggle ) * _RimlightEmissionIntensity ) + ( ( clampResult87_g1463 * _SpecularEmissiveToggle ) * _SpecularEmissionIntensity ) + ( ( clampResult90_g1463 * _BaseEmissiveToggle ) * _BaseEmissionIntensity ) + ( ( temp_output_43_0_g1463 * _Shadow1EmissiveToggle ) * _Shadow1EmissionIntensity ) + ( ( clampResult91_g1463 * _Shadow2EmissiveToggle ) * _Shadow2EmissionIntensity ) + ( ( 1.0 - ILMA357 ) * ( _ILMAlphaEmissionIntensity * _ILMAlphaLinesEmissionToggle ) ) );
				...
				float temp_output_48_0_g1465 = _DebugGroup;
				float temp_output_47_0_g1465 = _VertexChannel;
				float4 temp_output_59_0_g1465 = _DebugColor;
				float temp_output_50_0_g1465 = _ILMChannel;
				float3 ILM988 = linearToGamma277;
				float4 appendResult1031 = (float4(ILM988 , ILMA357));
				float4 temp_output_54_0_g1465 = appendResult1031;
				float4 break7_g1465 = temp_output_54_0_g1465;
				float temp_output_57_0_g1465 = 0.0;
				float4 Debug987 = ( temp_output_48_0_g1465 == 0.0 ? ( temp_output_47_0_g1465 == 0.0 ? i.ase_color : ( temp_output_47_0_g1465 == 1.0 ? ( i.ase_color.r * temp_output_59_0_g1465 ) : ( temp_output_47_0_g1465 == 2.0 ? ( i.ase_color.g * temp_output_59_0_g1465 ) : ( temp_output_47_0_g1465 == 3.0 ? ( i.ase_color.b * temp_output_59_0_g1465 ) : ( temp_output_47_0_g1465 == 4.0 ? ( i.ase_color.a * temp_output_59_0_g1465 ) : float4( 0,0,0,0 ) ) ) ) ) ) : ( temp_output_48_0_g1465 == 1.0 ? ( temp_output_50_0_g1465 == 0.0 ? temp_output_54_0_g1465 : ( temp_output_50_0_g1465 == 1.0 ? ( break7_g1465.r * temp_output_59_0_g1465 ) : ( temp_output_50_0_g1465 == 2.0 ? ( break7_g1465.g * temp_output_59_0_g1465 ) : ( temp_output_50_0_g1465 == 3.0 ? ( break7_g1465.b * temp_output_59_0_g1465 ) : ( temp_output_50_0_g1465 == 4.0 ? ( break7_g1465.a * temp_output_59_0_g1465 ) : float4( 0,0,0,0 ) ) ) ) ) ) : ( temp_output_48_0_g1465 == 2.0 ? ( temp_output_57_0_g1465 == 0.0 ? ( BaseA580 * temp_output_59_0_g1465 ) : ( temp_output_57_0_g1465 == 1.0 ? ( 0.0 * temp_output_59_0_g1465 ) : float4( 0,0,0,0 ) ) ) : float4( 0,0,0,0 ) ) ) );
		...
		Pass
		{
			...
			v2f vert ( appdata v )
			{
				...
				float temp_output_33_0_g1472 = _OutlineThickness;
				float3 temp_output_135_0_g1472 = ( ( v.ase_normal * 2E-05 ) * ( ( ( _EnableCameraDistanceMult * eyeDepth * v.ase_color.g * temp_output_33_0_g1472 ) + temp_output_33_0_g1472 ) * v.ase_color.a ) );
				float3 objectSpaceViewDir136_g1472 = ObjSpaceViewDir( float4( temp_output_135_0_g1472 , 0.0 ) );
				float4 normalizeResult142_g1472 = ASESafeNormalize( ( float4( objectSpaceViewDir136_g1472 , 0.0 ) - v.vertex ) );
				float clampResult141_g1472 = clamp( ( v.ase_color.b + _DepthOffset ) , 0.0 , 1.0 );
				float4 lerpResult145_g1472 = lerp( float4( temp_output_135_0_g1472 , 0.0 ) , -normalizeResult142_g1472 , ( 1.0 - clampResult141_g1472 ));
				...
				v.vertex.xyz += lerpResult145_g1472.xyz;
			...
			float4 frag (v2f i ) : SV_Target
			{
				...
				float4 lerpResult1010 = lerp( ILMAColor1005 , float4( temp_output_993_0[0] , 0.0 ) , ILMA357);
				float4 Base339 = ( ILMColorSetting1007 == 1.0 ? lerpResult1010 : float4( ( ILMA357 * temp_output_993_0[0] ) , 0.0 ) );
				float4 temp_output_13_0_g1472 = max( float4( 0,0,0,0 ) , ( 0.2 * Base339 ) );
				

				outOutlineColor = ( ( _OutlineEnableBaseColorMult > 0.0 ? ( temp_output_13_0_g1472 * temp_output_13_0_g1472 ) : _OutlineColor ) * _OutlineColorIntensity ).rgb;
...
/*ASEBEGIN
Version=18900
-2375;195;2298;1017;-6469.729;-4101.86;1.098875;True;True
...
Node;AmplifyShaderEditor.RangedFloatNode;912;7251.105,4997.887;Inherit;False;Property;_OutlineThickness;Outline Thickness;78;0;Create;True;0;0;0;True;0;False;1;1;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.CommentaryNode;555;-1765.585,4137.153;Inherit;False;1451.782;658.5377;;14;960;947;944;873;950;867;544;869;632;779;780;777;778;1065;Light Color Calculation;1,1,1,1;0;0
...
Node;AmplifyShaderEditor.FunctionNode;1033;152.7226,6096.615;Inherit;False;Debug;-1;;1465;6f93fc90e278e9a449a8e28c4cbb4ee4;0;8;48;FLOAT;0;False;59;COLOR;0,0,0,0;False;47;FLOAT;0;False;50;FLOAT;0;False;54;COLOR;0,0,0,0;False;57;FLOAT;0;False;55;FLOAT;0;False;56;FLOAT;0;False;1;COLOR;0
...
Node;AmplifyShaderEditor.FunctionNode;960;-1427.66,4325.012;Inherit;False;Light Color Calculation;-1;;1473;5b8f5c3f2a647dc4d89d921c89ebb0e5;0;7;25;FLOAT;0;False;10;FLOAT;0;False;5;FLOAT;0;False;14;FLOAT;0;False;6;COLOR;0,0,0,0;False;7;FLOAT;0;False;19;FLOAT;0;False;2;COLOR;0;COLOR;16
...
Node;AmplifyShaderEditor.FunctionNode;1053;7678.969,5104.001;Inherit;False;ASWOutline;-1;;1472;ea9f2d47913480c4f9e91454af7efa45;0;7;33;FLOAT;0;False;47;FLOAT;0;False;127;FLOAT;0.5;False;35;COLOR;0,0,0,0;False;37;FLOAT;0;False;36;FLOAT;0;False;31;COLOR;0,0,0,0;False;2;COLOR;0;FLOAT4;109
...
Node;AmplifyShaderEditor.FunctionNode;1064;2707.013,3668.041;Inherit;False;Final Dot Calculation;-1;;1458;966aa02f52a17a443a5106621d0d8d24;0;9;5;FLOAT;0;False;21;FLOAT;0;False;6;FLOAT;0;False;3;FLOAT;0;False;4;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;0;False;11;FLOAT;0;False;10;FLOAT;0;False;3;FLOAT;0;FLOAT;8;FLOAT;9
...
Node;AmplifyShaderEditor.FunctionNode;1035;4243.784,3442.179;Inherit;False;Emission - Guilty Gear Xrd;-1;;1464;0804f3eb9e8a1224897e481e37ef96e3;0;13;17;FLOAT;0;False;22;FLOAT;0;False;20;FLOAT;0;False;6;FLOAT;0;False;7;FLOAT;0;False;8;FLOAT;0;False;9;FLOAT;0;False;10;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;4;FLOAT;0;False;5;FLOAT;0;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.FunctionNode;965;3472,3680;Inherit;False;Ramp Creation - Guilty Gear Xrd;-1;;1463;92a8a58660488504fb4c65a454407910;0;17;1;FLOAT;0;False;3;FLOAT;0;False;2;FLOAT;0;False;5;FLOAT;0;False;7;FLOAT;0;False;80;FLOAT;0;False;8;FLOAT;0;False;9;FLOAT;0;False;10;FLOAT;0;False;11;FLOAT;0;False;12;FLOAT;0;False;13;FLOAT;0;False;97;FLOAT;0;False;101;FLOAT;0;False;104;FLOAT;0;False;107;FLOAT;0;False;110;FLOAT;0;False;10;FLOAT;100;FLOAT;103;FLOAT;106;FLOAT;109;FLOAT;112;FLOAT;76;FLOAT;69;FLOAT;62;FLOAT;0;FLOAT;44
Node;AmplifyShaderEditor.FunctionNode;637;1422.762,3657.256;Inherit;False;Dot Creation;-1;;1460;fd90efbfe94791944bc1c5b55d8d67e4;0;7;12;FLOAT;0;False;17;FLOAT;0;False;16;FLOAT;0;False;29;FLOAT;0;False;28;FLOAT;0;False;33;FLOAT;0;False;34;FLOAT;0;False;3;FLOAT;0;FLOAT;42;FLOAT;52
...
Node;AmplifyShaderEditor.TemplateMultiPassMasterNode;1069;7467.201,4094.323;Float;False;False;-1;2;ASEMaterialInspector;100;1;New Amplify Shader;fd5163ddf7350f946aff871fd3d7e3fe;True;ShadowCaster;0;4;ShadowCaster;0;False;False;False;False;False;False;False;False;False;False;False;False;False;False;True;2;False;-1;False;False;False;False;False;False;False;False;False;False;False;False;False;False;True;1;RenderType=Opaque=RenderType;True;2;0;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;True;1;False;-1;True;3;False;-1;False;True;1;LightMode=ShadowCaster;False;0;;0;0;Standard;0;False;0
...
Node;AmplifyShaderEditor.TemplateMultiPassMasterNode;1066;8426.12,4375.02;Float;False;True;-1;2;ASWGuiltyGearXrdGUI;100;8;.Aerthas/Arc System Works/Game Select/Guilty Gear Xrd/Opaque v7.2.0;fd5163ddf7350f946aff871fd3d7e3fe;True;ForwardBase;0;0;ForwardBase;3;False;False;False;False;False;False;False;False;False;False;False;False;False;True;True;1;False;-1;False;False;False;False;False;False;False;False;False;False;False;False;False;False;True;1;RenderType=Opaque=RenderType;True;2;0;False;True;0;1;False;-1;0;False;-1;0;1;False;-1;0;False;-1;True;0;False;-1;0;False;-1;False;False;False;False;False;False;False;False;False;True;0;False;-1;True;True;0;False;-1;False;True;True;True;True;True;0;False;-1;False;False;False;False;False;False;False;True;False;255;False;-1;255;False;-1;255;False;-1;7;False;-1;1;False;-1;1;False;-1;1;False;-1;7;False;-1;1;False;-1;1;False;-1;1;False;-1;False;True;1;False;-1;True;3;False;-1;True;True;0;False;-1;0;False;-1;True;1;LightMode=ForwardBase;True;2;0;;0;0;Standard;0;0;5;True;True;False;True;False;False;;False;0
...
ASEEND*/
//CHKSM=1A94A73D35AA1920B697555B4603F85FD328CD75